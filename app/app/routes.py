...
def load_last_match():
    saved_matches = [
        ...
        for root, dirs, files in os.walk(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "matches"),
            topdown=False,
        )
    ][0]
    if saved_matches:
        ...
        filename = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "matches", last_match
        )
        with open(filename, "r") as file:
            ...
            return make_response(jsonify(MATCH.to_json()), 200)
    return abort(404)
...
def create_match():
    ...
    player_1 = Player(
        url=player_1_data["id"],
        firstName=player_1_data["first_name"],
        lastName=player_1_data["last_name"],
        member_id=player_1_data["member_id"],
    )
    player_2 = Player(
        url=player_2_data["id"],
        firstName=player_2_data["first_name"],
        lastName=player_2_data["last_name"],
        member_id=player_2_data["member_id"],
    )
    MATCH = Match(
        url=url,
        player1=player_1,
...
def save_match():
    filename = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "matches", MATCH.filename
    )
...
def undo():
    ...
    if undone_balls:
        save_match()
        return jsonify([pot.to_json() for pot in undone_balls])
...
def end_frame():
    return render_template(
        "end_frame.html",
        stats=MATCH.currentFrame().stats(),
        frame=True,
        frame_number=MATCH.currentFrameId(),
